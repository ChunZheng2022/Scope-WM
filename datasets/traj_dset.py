...
# Adapted from PyTorch torch.utils.data dataset helpers.
...
class TrajDataset(Dataset, abc.ABC):
    ...
    def get_action_slice(self, idx, start, end):
        if hasattr(self, "actions"):
            return self.actions[idx, start:end]
        _, act, _, _ = self.get_frames(idx, range(start, end))
        return act
...
class TrajSubset(TrajDataset, Subset):
    ...
    def get_action_slice(self, idx, start, end):
        if hasattr(self.dataset, "get_action_slice"):
            return self.dataset.get_action_slice(self.indices[idx], start, end)
        if hasattr(self.dataset, "actions"):
            return self.dataset.actions[self.indices[idx], start:end]
        _, act, _, _ = self.get_frames(idx, range(start, end))
        return act
...
class TrajSlicerDataset(TrajDataset):
    ...
    def __getitem__(self, idx):
        ...
        frame_indices = range(start, end, self.frameskip)
        if hasattr(self.dataset, "get_frames"):
            obs, _, state, _ = self.dataset.get_frames(i, frame_indices)
            act = self.dataset.get_action_slice(i, start, end)
        else:
